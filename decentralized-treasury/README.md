...
| votingLedgerScheduler | object | `{"affinity":{},"deploymentAnnotations":{},"enabled":true,"extraEnvVars":[],"image":{"pullPolicy":"","repository":"minafoundation/dt-voting-ledger-scheduler","tag":""},"lifecycleIds":[],"nodeSelector":{},"periodsPerLifecycle":4,"podAnnotations":{},"pollIntervalSeconds":30,"replicaCount":1,"resources":{},"sqlite":{"keepLastN":0},"stakingLedgers":{"bucket":"http://staking-ledgers-fetcher:8080","lifecycleLedgerHashes":"","prefix":"","source":"http","strictAlignment":false},"stakingLedgersDirectory":"/staking-ledgers","stakingLedgersExistingClaim":"","stakingLedgersKeepLastN":1,"stakingLedgersSizeLimit":"","stakingLedgersSyncIntervalSeconds":300,"tolerations":[]}` | Builds voting ledgers from staking ledger archives. The sole producer of the lifecycle SQLite databases, and the only workload that never touches Postgres. |
...
| votingLedgerScheduler.stakingLedgers.bucket | string | `"http://staking-ledgers-fetcher:8080"` | Where ledgers come from. For `http` a base URL; for `gcs`/`s3` a bucket name (devnet GCS: mina-staking-ledgers-devnet; mainnet: mina-staking-ledgers, which has no -mainnet suffix). |
