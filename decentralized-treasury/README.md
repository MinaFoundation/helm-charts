...
| backoffice.auth.basic.existingSecret | string | `""` | An htpasswd Secret you manage instead, with the content under the `auth` key. Mutually exclusive with users. |
| backoffice.auth.basic.realm | string | `"Treasury break-glass"` | Realm shown in the browser's credential prompt |
...
| backoffice.auth.mode | string | `""` | One of:   basic    - the chart renders an htpasswd Secret and the nginx              basic-auth annotations. nginx ingress only; ALB does not              implement them.   external - you supply the annotations in backoffice.ingress.annotations              (oauth2-proxy, ALB OIDC, an auth proxy of your own).   none     - published with no authentication, deliberately.  Empty is not valid once the console is routed: the chart fails rather than pick one for you. Ignored entirely while it is unexposed, which is the default - `kubectl port-forward` is already gated by cluster RBAC. |
