...
![Version: 0.4.1-speedrun](https://img.shields.io/badge/Version-0.4.1--speedrun-informational?style=flat-square) ![Type: application](https://img.shields.io/badge/Type-application-informational?style=flat-square) ![AppVersion: 0.1.0](https://img.shields.io/badge/AppVersion-0.1.0-informational?style=flat-square)
...
| lifecycleFanout | object | `{"deploymentAnnotations":{},"enabled":false,"groupSize":1,"podAnnotations":{},"pollIntervalSeconds":60,"resources":{}}` | EXPERIMENTAL, off by default. Fans out one real, fully-proven lifecycle's proofs across a group of synthetic sibling lifecycle ids that share the same underlying staking ledger, so the expensive build+prove step runs once per group instead of once per synthetic id. A sibling's (potentially large) sqlite body is never duplicated - only a small pointer object naming its canonical, which every consumer's s3-sync-pull sidecar resolves into a local symlink at fetch time, so N siblings cost one body on disk, not N. Intended for accelerated lifecycle testing (e.g. many short lifecycles inside one real Mina epoch), never for a production release. |
