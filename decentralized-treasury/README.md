...
| votingLedgerScheduler | object | `{"affinity":{},"deploymentAnnotations":{},"enabled":true,"extraEnvVars":[],"image":{"pullPolicy":"","repository":"minafoundation/dt-voting-ledger-scheduler","tag":""},"lifecycleIds":[],"nodeSelector":{},"periodsPerLifecycle":4,"podAnnotations":{},"pollIntervalSeconds":30,"replicaCount":1,"resources":{},"sqlite":{"keepLastN":0},"stakingLedgers":{"bucket":"mina-staking-ledgers-devnet","lifecycleLedgerHashes":"","prefix":"staking-","source":"gcs","strictAlignment":false},"stakingLedgersDirectory":"/staking-ledgers","stakingLedgersExistingClaim":"","stakingLedgersKeepLastN":1,"stakingLedgersSizeLimit":"","stakingLedgersSyncIntervalSeconds":300,"tolerations":[]}` | Builds voting ledgers from staking ledger archives. The sole producer of the lifecycle SQLite databases, and the only workload that never touches Postgres. |
...
| votingLedgerScheduler.stakingLedgers.bucket | string | `"mina-staking-ledgers-devnet"` | Bucket holding the staking ledgers. devnet: mina-staking-ledgers-devnet; mainnet: mina-staking-ledgers (note there is no -mainnet suffix). |
...
| votingLedgerScheduler.stakingLedgers.prefix | string | `"staking-"` | Object-name prefix to list under. Empty defaults to `staking-` for gcs and to `.Values.network` for s3. |
| votingLedgerScheduler.stakingLedgers.source | string | `"gcs"` | Where staking ledgers come from: `gcs` (public Google Cloud Storage over anonymous HTTPS, objects named `staking-<epoch>-<ledgerHash>-<md5>-<date>_<time>.json`) or `s3` (the legacy AWS layout, `<epoch>-<hash>.tar.gz`). The produced-lifecycle check is always AWS, so IRSA is required either way. |
