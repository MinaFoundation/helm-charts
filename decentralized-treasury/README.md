...
![Version: 0.6.0-speedrun](https://img.shields.io/badge/Version-0.6.0--speedrun-informational?style=flat-square) ![Type: application](https://img.shields.io/badge/Type-application-informational?style=flat-square) ![AppVersion: 0.1.0](https://img.shields.io/badge/AppVersion-0.1.0-informational?style=flat-square)
...
| backoffice | object | `{"auth":{"basic":{"existingSecret":"","realm":"Treasury break-glass","users":{}},"mode":""},"deploymentAnnotations":{},"enabled":false,"extraEnvVars":[],"image":{"pullPolicy":"","repository":"minafoundation/dt-backoffice","tag":""},"ingress":{"annotations":{},"tls":[]},"livenessProbe":{"failureThreshold":3,"httpGet":{"path":"/","port":"http"},"initialDelaySeconds":20,"periodSeconds":20,"timeoutSeconds":5},"pathPrefix":"","podAnnotations":{},"port":3200,"publicBaseUrl":"","publicEnv":{"NEXT_PUBLIC_EMPTY_NULLIFIER_ROOT":"","NEXT_PUBLIC_EMPTY_VOTING_LEDGER_ROOT":"","NEXT_PUBLIC_NETWORK_ID":"DEVNET","NEXT_PUBLIC_STAKING_LEDGER_TO_VOTING_LEDGER_VERIFICATION_KEY_JSON":"","NEXT_PUBLIC_TREASURY_PROPOSAL_VERIFICATION_KEY_JSON":"","NEXT_PUBLIC_VOTE_REDUCER_VERIFICATION_KEY_JSON":""},"readinessProbe":{"failureThreshold":3,"httpGet":{"path":"/","port":"http"},"initialDelaySeconds":10,"periodSeconds":10,"timeoutSeconds":5},"replicaCount":1,"resources":{},"service":{"annotations":{},"port":3200,"type":"ClusterIP"}}` | Break-glass operator console (apps/backoffice). A separate image from web, and necessarily a separate hostname rather than a path: the app has no Next.js basePath, so its HTML references /_next/* at the root and it cannot be served under a prefix such as /backoffice by any amount of ingress rewriting. Putting it on a path would mean setting basePath at BUILD time, which bakes that prefix into the image.  Enabling this creates a Deployment and a ClusterIP Service only. Reaching it is a deliberate second step: either set ingress.hosts.backoffice, which works in every ingress mode, or leave it unexposed and use `kubectl port-forward`, which is the safer default for a console that can pause the treasury.  A port-forward is enough to load the page but not to use it. The console talks to the Mina node from the browser, and its POSTs are content-type: application/json, so they are preflighted - a node on another origin has to answer CORS or every call fails, and the app reports it as "Treasury owner account was not found". Setting ingress.hosts.backoffice in `proxy` mode is what solves that: the proxy serves /mina on the console's own origin, so nothing is cross-origin and the daemon needs no CORS at all. |
| backoffice.auth | object | `{"basic":{"existingSecret":"","realm":"Treasury break-glass","users":{}},"mode":""}` | Authentication in front of the console. The app has no login of its own: it authenticates the five *signers* by their Ledger devices, not whoever opens the page, and an unauthenticated visitor can still reach the pause and emergency-withdrawal flows. So whatever fronts it is the only thing between the internet and those buttons, and the chart refuses to publish it until this is set deliberately. |
| backoffice.auth.basic.existingSecret | string | `""` | An htpasswd Secret you manage instead, with the content under the `auth` key. Mutually exclusive with users. Read by whichever component enforces the auth: the proxy pod mounts it as a file, an nginx ingress controller reads it by name. |
| backoffice.auth.basic.realm | string | `"Treasury break-glass"` | Realm shown in the browser's credential prompt, and the `auth_basic` value when the in-cluster proxy is what enforces it |
| backoffice.auth.basic.users | object | `{}` | Users as {name: password}, hashed into a Secret with bcrypt. The plaintext still lives wherever these values do, so prefer existingSecret with a secret-manager reference for anything lasting. |
| backoffice.auth.mode | string | `""` | One of:   basic    - the chart renders an htpasswd Secret. What enforces it              depends on how the console is routed:               * ingress.mode `proxy` with ingress.hosts.backoffice -                the in-cluster nginx serves that hostname and applies                auth_basic itself. This works behind an ALB, because                the load balancer only forwards the host and enforces                nothing. The recommended shape.              * any other mode - the chart sets the nginx ingress                basic-auth annotations instead, which needs an nginx                ingress controller. Refused with className `alb`,                which does not implement them and would publish the                console unauthenticated.   external - you supply the annotations in backoffice.ingress.annotations              (oauth2-proxy, ALB OIDC, an auth proxy of your own). Needs              the console to own an Ingress object, so it is refused in              `proxy` mode, where the proxy owns the hostname instead.   none     - published with no authentication, deliberately. Loading the              page reveals nothing that is not already on chain, and every              break-glass action still needs signer keys - but it is a              public page with a pause button on it, so treat it as a              temporary state.  Empty is not valid once the console is routed: the chart fails rather than pick one for you. Ignored entirely while it is unexposed, which is the default - `kubectl port-forward` is already gated by cluster RBAC. |
| backoffice.deploymentAnnotations | object | `{}` | Annotations to add to the deployment |
| backoffice.enabled | bool | `false` | Whether to deploy the break-glass console. Off by default: it requires config.multisigParticipantsPublicKeys, which has no sensible default, and an upgrade should not silently add a workload that cannot work. |
| backoffice.extraEnvVars | list | `[]` | Additional environment variables |
| backoffice.image.pullPolicy | string | `""` | Overrides the chart-wide pull policy |
| backoffice.image.repository | string | `"minafoundation/dt-backoffice"` | The image repository |
| backoffice.image.tag | string | `""` | Overrides the chart-wide tag |
| backoffice.ingress | object | `{"annotations":{},"tls":[]}` | The console's own Ingress object, separate from the web and API ones because annotations apply per object: authentication put on the shared object would gate the public web app too. |
| backoffice.ingress.annotations | object | `{}` | Annotations for the console's Ingress. This is where an `external` authentication layer is configured - the chart adds nothing of its own beyond the basic-auth trio, and entries here win over that.  oauth2-proxy behind an nginx ingress:   nginx.ingress.kubernetes.io/auth-url: https://oauth.example.org/oauth2/auth   nginx.ingress.kubernetes.io/auth-signin: https://oauth.example.org/oauth2/start?rd=$escaped_request_uri  ALB with OIDC, which the controller enforces at the load balancer:   alb.ingress.kubernetes.io/auth-type: oidc   alb.ingress.kubernetes.io/auth-idp-oidc: '{"issuer":"...","authorizationEndpoint":"...","tokenEndpoint":"...","userInfoEndpoint":"...","secretName":"backoffice-oidc"}'   alb.ingress.kubernetes.io/auth-on-unauthenticated-request: authenticate   alb.ingress.kubernetes.io/group.name: decentralized-treasury |
| backoffice.ingress.tls | list | `[]` | TLS for the console's hostname. Empty falls back to ingress.tls, which is usually right only if that certificate already covers the hostname. |
| backoffice.livenessProbe | object | `{"failureThreshold":3,"httpGet":{"path":"/","port":"http"},"initialDelaySeconds":20,"periodSeconds":20,"timeoutSeconds":5}` | Liveness probe |
| backoffice.pathPrefix | string | `""` | Serve the console under a path on the main hostname, the way /sqlite and /proofs are served, instead of on its own subdomain. Empty (the default) means path routing is off.  This ONLY works with a dt-backoffice image built with a matching Next.js basePath, because Next resolves basePath at build time and writes it into the asset URLs in its HTML. The two must agree exactly: a stock image (basePath unset) behind this route 404s the page, and a `/backoffice` image reached at the root 404s its assets. Nothing in the routing layer can reconcile a mismatch - that is why this is off by default.  Routing is deliberately non-stripping, so the prefix survives the hop and the app receives the path it was built for. In ingress mode `proxy` the in-cluster nginx serves it; in `path` and `host` mode the chart adds a non-rewriting path rule to the web Ingress. |
| backoffice.podAnnotations | object | `{}` | Annotations to add to the pods |
| backoffice.port | int | `3200` | Port the container listens on |
| backoffice.publicBaseUrl | string | `""` | Public origin the browser reaches the console on, used to derive NEXT_PUBLIC_MINA_NODE_URL as `<publicBaseUrl>/mina/graphql`.  Setting this requires something to serve /mina on that same origin. With proxy.enabled the chart arranges it: the ingress.hosts.backoffice hostname gets a /mina rule of its own, pointed at the in-cluster proxy, so the call stays same-origin and the daemon needs no CORS. Without the proxy the derived URL would hit the console's own Next server, so the chart refuses to render - set NEXT_PUBLIC_MINA_NODE_URL in publicEnv instead and point it straight at a node.  Whatever fronts this must serve HTTPS. The app sets Cross-Origin-Embedder-Policy: require-corp, and its proving worker needs a secure context - over plain HTTP the browser refuses and signing fails. |
| backoffice.publicEnv | object | `{"NEXT_PUBLIC_EMPTY_NULLIFIER_ROOT":"","NEXT_PUBLIC_EMPTY_VOTING_LEDGER_ROOT":"","NEXT_PUBLIC_NETWORK_ID":"DEVNET","NEXT_PUBLIC_STAKING_LEDGER_TO_VOTING_LEDGER_VERIFICATION_KEY_JSON":"","NEXT_PUBLIC_TREASURY_PROPOSAL_VERIFICATION_KEY_JSON":"","NEXT_PUBLIC_VOTE_REDUCER_VERIFICATION_KEY_JSON":""}` | Browser-facing configuration, read at container start. Entries here win over the values derived from publicBaseUrl and config.  Unlike web, no treasury/indexer/processor API URLs are derived: the console builds and submits transactions against the Mina node directly, and those URLs would only ever surface in its settings dialog. |
| backoffice.readinessProbe | object | `{"failureThreshold":3,"httpGet":{"path":"/","port":"http"},"initialDelaySeconds":10,"periodSeconds":10,"timeoutSeconds":5}` | Readiness probe |
| backoffice.replicaCount | int | `1` | Replica count. Stateless - every operation is driven from the browser. |
| backoffice.resources | object | `{}` | The Resources |
| backoffice.service.annotations | object | `{}` | Annotations to add to the service |
| backoffice.service.port | int | `3200` | The service port |
| backoffice.service.type | string | `"ClusterIP"` | The service type |
...
| config.minaNodeUrl | string | `""` | Mina daemon GraphQL endpoint, read server-side by the staking-ledgers sync. It supplies slotSinceGenesis, slotsPerEpoch and the authoritative stakingEpochData.ledger.hash, which is what makes ledger selection survive a hardfork. Distinct from proxy.minaNodeUpstream, which is browser-facing. |
| config.multisigParticipantsPublicKeys | list | `[]` | The five break-glass multisig participants, in the SAME order as the pause-controller deployment. Public keys only - the signers hold their own private keys, on Ledger devices, and nothing here ever needs them.  The contract is fixed at 3-of-5 in the circuit, so this is exactly five entries: the set cannot grow without changing the verification key. Order is load-bearing, because the on-chain commitment is an ordered hash of the five and signature i is checked against participant i.  Rotating signers is an on-chain operation (`pause-controller rotate-multisig-keys`, authorised by 3 of the *current* five), not a redeploy. Change this list to match after a rotation, never before.  Required by backoffice.enabled, and unused by every other component. |
...
| config.treasuryDeployedAtSlot | int | `0` | Slot at which the treasury contract was deployed, on the `globalSlotSinceGenesis` scale (the one the contract's `globalSlotSinceGenesis.requireBetween` gates on, which does NOT reset at a hardfork). Lifecycle ids are counted from here.  MUST sit on a Mina epoch boundary. Every proposal in a lifecycle has to snapshot the same stakingEpochData, which holds only if the proposal period lies inside a single epoch. On a chain that has hardforked, epoch boundaries are offset from multiples of slotsPerEpoch, so:    treasuryDeployedAtSlot % slotsPerEpoch == hardforkOffset % slotsPerEpoch   where hardforkOffset = slotSinceGenesis - (epoch * slotsPerEpoch + slot)  Get this wrong and the back half of each proposal period is silently unprovable. The sync checks it on every start and warns (or aborts, with votingLedgerScheduler.stakingLedgers.strictAlignment). |
...
| ingress.hosts | object | `{"api":"","backoffice":"","indexer":"","processor":""}` | Per-service hostnames. api, indexer and processor apply only when mode is `host`; backoffice applies in every mode. Any left empty is simply not exposed. |
| ingress.mode | string | `"path"` | Routing mode.   path  - one hostname, with /api, /indexer and /processor stripped before           proxying (needs a rewrite annotation; works on nginx).   host  - a separate hostname per service and no rewriting. Use this on ALB:           the AWS Load Balancer Controller has no rewrite-target equivalent,           so path mode cannot strip the prefix and the APIs would 404.   proxy - one hostname routed to the in-cluster proxy, which does the           stripping itself. Also for ALB, and the one to use when the           browser should stay on a single origin. Requires proxy.enabled. |
| ingress.tls | list | `[]` | TLS configuration, e.g. from cert-manager |
| lifecycleFanout | object | `{"enabled":false,"groupSize":1,"siblingWindowAhead":1,"siblingWindowBehind":0}` | EXPERIMENTAL, off by default. Groups lifecycle ids that share one Mina epoch's staking ledger, so the expensive build+prove step runs once per group instead of once per id. Only the canonical id of each group (id % groupSize == 0) is built, proven and published; the others are served by each consumer relabelling a local copy of that canonical's body.  Nothing is published for a non-canonical id - no body, no marker, no pointer object. A body cannot simply be shared under another id's name: its rows are namespaced with the lifecycle id that produced them ("staking-ledger-42-accounts:7"), so a reader opening it as a different id finds none of its own keys and serves an empty ledger, and every proposal in that lifecycle fails the staking ledger root check. An earlier revision published <id>.sqlite.alias pointers that consumers symlinked, and it failed exactly that way, silently.  The cost is one copy of a body per materialised id on each consumer's volume, bounded by the sibling window below. Intended for accelerated lifecycle testing (e.g. many short lifecycles inside one real Mina epoch), never for a production release. |
| lifecycleFanout.enabled | bool | `false` | Groups lifecycle ids: restricts the voting-ledger-scheduler to canonical ids and lets api and processor materialise the others locally. Leave false for every normal release. |
| lifecycleFanout.groupSize | int | `1` | Number of consecutive lifecycle ids per group. 1 (the default) means every id is its own group, i.e. no grouping at all. Must be chosen so group boundaries land on real Mina epoch boundaries (requires config.treasuryDeployedAtSlot to be epoch-aligned, e.g. 0) - otherwise a group can straddle two different real staking-ledger hashes and the relabelled bodies carry the wrong root. |
| lifecycleFanout.siblingWindowAhead | int | `1` | How many lifecycles ahead of the one the chain is in to materialise, so the next one is ready before it opens. Each id in the window costs one copy of a body on every consumer's volume. |
| lifecycleFanout.siblingWindowBehind | int | `0` | How many lifecycles behind the one the chain is in to keep materialised. 0 keeps only the current one; raise it to keep serving witnesses for a lifecycle whose voting or tally period has just passed. |
...
| network | string | `"devnet"` | Mina network this release targets. Doubles as the S3 key prefix, which is what keeps lifecycle ids from colliding between networks: a lifecycle id is a small integer counted from TREASURY_DEPLOYED_AT_SLOT, so every network starts at 0. |
...
| processor | object | `{"affinity":{},"batchSize":"","deploymentAnnotations":{},"enabled":true,"extraEnvVars":[],"image":{"pullPolicy":"","repository":"minafoundation/dt-processor","tag":""},"persistence":{"enabled":false,"size":"40Gi","storageClass":""},"podAnnotations":{},"pollIntervalMs":"","replicaCount":1,"resources":{},"sqlite":{"enabled":true,"keepLastN":0}}` | Event processor worker (apps/api start:processor). Consumes events from indexer-api and writes proposal state to Postgres. |
...
| proving.scheduler.server.enabled | bool | `true` | Serve the sqlite cache and proofs over HTTP from this pod |
| proving.scheduler.server.image.pullPolicy | string | `"IfNotPresent"` | Image pull policy |
| proving.scheduler.server.image.repository | string | `"nginxinc/nginx-unprivileged"` | Image for the serving container |
| proving.scheduler.server.image.tag | string | `"1.27-alpine"` | Image tag |
| proving.scheduler.server.port | int | `8080` | Port the artefacts are served on. 8080 because the unprivileged nginx image cannot bind a privileged port. |
| proving.scheduler.server.resources | object | `{}` | Resources for the serving container |
| proving.scheduler.server.securityContext | object | `{"allowPrivilegeEscalation":false,"capabilities":{"drop":["ALL"]},"runAsNonRoot":true,"runAsUser":101}` | Security context for the serving container |
...
| proving.worker.autoscale | object | `{"enabled":false,"maxReplicas":3,"minReplicas":0,"pollIntervalSeconds":15,"resources":{},"scaleDownAfterSeconds":180}` | Scale the workers from the depth of the proving queue.  Proving is by far the most expensive workload here, and it is idle whenever there is nothing to prove - but idle replicas still hold their CPU and memory requests and so keep nodes alive. A sidecar on proving-scheduler watches the queue and scales this Deployment between minReplicas and maxReplicas.  The chart stops rendering `replicas` when this is on, so that applying the release does not keep resetting the count the sidecar chose. |
...
| proving.worker.autoscale.maxReplicas | int | `3` | Ceiling on the proving cluster.  Deliberately conservative: each replica takes a whole node (the worker sets podAntiAffinity and a 16 vCPU floor), so this is the one value here that turns into machines. A deployment that wants a wider proving cluster has to ask for it, sized against its own ledger, rather than inheriting a ceiling that quietly provisions a fleet.  Measured cost per proof to size it against: ~39s per digest proof and ~19s per merge proof, with batches = ceil(accounts / ACCOUNT_BATCH_SIZE). Raising this stops helping once proving drops below the single-threaded trace-digest that has to finish first. |
...
| proxy.extraServices | list | `[]` | Extra upstreams to expose, as a list of {key, host, port}. Rendered exactly like the built-in /api, /indexer and /processor routes. Use it to front something outside this release - e.g. the staking ledgers provider:    extraServices:     - key: staking-ledgers       host: mina-staking-ledgers-provider       port: 8080 |
...
| votingLedgerScheduler | object | `{"affinity":{},"checkpoint":{"intervalIndices":0},"deploymentAnnotations":{},"enabled":true,"extraEnvVars":[],"image":{"pullPolicy":"","repository":"minafoundation/dt-voting-ledger-scheduler","tag":""},"lifecycleIds":[],"nodeSelector":{},"periodsPerLifecycle":4,"podAnnotations":{},"pollIntervalSeconds":30,"replicaCount":1,"resources":{},"sqlite":{"keepLastN":0},"stakingLedgers":{"bucket":"http://mina-staking-ledgers-provider:8080","lifecycleLedgerHashes":"","prefix":"","source":"http","strictAlignment":false},"stakingLedgersDirectory":"/staking-ledgers","stakingLedgersExistingClaim":"","stakingLedgersKeepLastN":1,"stakingLedgersSizeLimit":"","stakingLedgersSyncIntervalSeconds":300,"tolerations":[]}` | Builds voting ledgers from staking ledger archives. The sole producer of the lifecycle SQLite databases, and the only workload that never touches Postgres. |
| votingLedgerScheduler.affinity | object | `{}` | Affinity for this workload alone. Empty follows the chart-wide `affinity`. |
| votingLedgerScheduler.checkpoint.intervalIndices | int | `0` | Indices of trace-digest progress between S3 checkpoints. 0 (the default) disables checkpointing entirely: an interrupted trace-digest restarts from index 0, as before. A positive value bounds how much work a preemption can cost - e.g. 500 indices at a lifecycle's measured rate is on the order of tens of minutes, not the full multi-hour run - which is what makes running this workload on spot capacity viable. Checkpoints live under a `.checkpoints/` sub-prefix of the sqlite bucket and are deleted once a lifecycle's `.done` marker is written. |
...
| votingLedgerScheduler.lifecycleIds | list | `[]` | Specific lifecycle ids to build, replacing the window above entirely. Only these are fetched, however far back they are, which makes filling a known gap deliberate rather than a side effect of widening the window.  Idempotent: a lifecycle whose voting ledger is already in the SQLite bucket is skipped, so a list can be left in place across deploys and will simply go quiet once its entries are built.  e.g. lifecycleIds: [283, 284] |
| votingLedgerScheduler.nodeSelector | object | `{}` | Node selector for this workload alone. Empty follows the chart-wide `nodeSelector`. Worth setting separately because `trace-digest` is the one single-threaded step on the critical path: it wants sustained single-core speed and an uninterruptible node, which is the opposite of what the proving workers want.  Replaces the chart-wide value rather than merging with it, so restate everything that still applies. |
...
| votingLedgerScheduler.stakingLedgers.bucket | string | `"http://mina-staking-ledgers-provider:8080"` | Where ledgers come from. For `http` a base URL; for `gcs`/`s3` a bucket name (devnet GCS: mina-staking-ledgers-devnet; mainnet: mina-staking-ledgers, which has no -mainnet suffix).  The default assumes the provider release sets `fullnameOverride: mina-staking-ledgers-provider`, since Helm would otherwise prefix the Service with its release name. Set this explicitly if it does not. |
| votingLedgerScheduler.stakingLedgers.lifecycleLedgerHashes | string | `""` | Pin specific lifecycles to specific ledger hashes, e.g. "12=jx...,13=jw...". Highest precedence, and the only way to resolve a lifecycle whose proposal period predates a hardfork - epoch numbers are ambiguous across one, so nothing else can identify the ledger. |
| votingLedgerScheduler.stakingLedgers.prefix | string | `""` | Object-name prefix to list under. Ignored for `http`. Empty defaults to `staking-` for gcs and to `.Values.network` for s3. |
| votingLedgerScheduler.stakingLedgers.source | string | `"http"` | Transport for staking ledgers:   `http` an in-cluster mina-staking-ledgers-provider, listed via nginx          autoindex JSON. Nothing leaves the cluster.   `gcs`  public Google Cloud Storage over anonymous HTTPS.   `s3`   AWS, for releases whose ledgers already live there. Object naming is handled uniformly rather than per transport - the ledger hash is located by pattern, so all published layouts are readable. The produced-lifecycle check is always AWS, so IRSA is needed regardless. |
| votingLedgerScheduler.stakingLedgers.strictAlignment | bool | `false` | Abort instead of warning when treasuryDeployedAtSlot is not on an epoch boundary, or when a lifecycle period is not exactly one Mina epoch. Left false so a bespoke network cannot be bricked by the check. |
| votingLedgerScheduler.stakingLedgersDirectory | string | `"/staking-ledgers"` | Where the staking ledger store is mounted. Holds <ledgerHash>.json payloads plus lifecycle-<id>.hash pointers, both written by the sync. |
...
| votingLedgerScheduler.stakingLedgersKeepLastN | int | `1` | How far back from the newest lifecycle to look, counted in lifecycles. The sync fetches only archives beginning a lifecycle in that window whose voting ledger is not in the SQLite bucket yet, and discards each one as soon as that voting ledger is published.  1 is the default and means the latest available lifecycle only. Anchoring to the newest is what stops the sync and the scheduler walking backwards through the whole backlog together; 0 opts into exactly that, and is only sensible on a network where a ledger takes minutes rather than hours. To rebuild specific older lifecycles, name them in `lifecycleIds` instead. Matches the script default, so the two cannot drift. |
...
| votingLedgerScheduler.tolerations | list | `[]` | Tolerations for this workload alone. Empty follows the chart-wide `tolerations`. Restate these whenever `nodeSelector` targets a tainted pool. |
| web | object | `{"deploymentAnnotations":{},"enabled":true,"extraEnvVars":[],"image":{"pullPolicy":"","repository":"minafoundation/dt-web","tag":""},"livenessProbe":{"failureThreshold":3,"httpGet":{"path":"/","port":"http"},"initialDelaySeconds":20,"periodSeconds":20,"timeoutSeconds":5},"podAnnotations":{},"port":3100,"publicBaseUrl":"","publicEnv":{"NEXT_PUBLIC_EMPTY_NULLIFIER_ROOT":"","NEXT_PUBLIC_EMPTY_VOTING_LEDGER_ROOT":"","NEXT_PUBLIC_NETWORK_ID":"DEVNET","NEXT_PUBLIC_SLOT_DURATION_MS":"90000","NEXT_PUBLIC_STAKING_LEDGER_TO_VOTING_LEDGER_VERIFICATION_KEY_JSON":"","NEXT_PUBLIC_TREASURY_PROPOSAL_VERIFICATION_KEY_JSON":"","NEXT_PUBLIC_VOTE_REDUCER_VERIFICATION_KEY_JSON":""},"readinessProbe":{"failureThreshold":3,"httpGet":{"path":"/","port":"http"},"initialDelaySeconds":10,"periodSeconds":10,"timeoutSeconds":5},"replicaCount":1,"resources":{},"service":{"annotations":{},"port":3100,"type":"ClusterIP"}}` | Next.js frontend (apps/web). Built from the `web` stage of the Dockerfile, so it is a different image from every other service. |
...
| web.image.repository | string | `"minafoundation/dt-web"` | The image repository |
...
| web.publicEnv | object | `{"NEXT_PUBLIC_EMPTY_NULLIFIER_ROOT":"","NEXT_PUBLIC_EMPTY_VOTING_LEDGER_ROOT":"","NEXT_PUBLIC_NETWORK_ID":"DEVNET","NEXT_PUBLIC_SLOT_DURATION_MS":"90000","NEXT_PUBLIC_STAKING_LEDGER_TO_VOTING_LEDGER_VERIFICATION_KEY_JSON":"","NEXT_PUBLIC_TREASURY_PROPOSAL_VERIFICATION_KEY_JSON":"","NEXT_PUBLIC_VOTE_REDUCER_VERIFICATION_KEY_JSON":""}` | Browser-facing configuration, read at container start rather than baked into the image, so one dt-web tag serves any network. Entries set here win over the values derived from publicBaseUrl. |
