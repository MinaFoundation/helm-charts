...
#
# With one lifecycle period per epoch this is equivalent to the literal
# `lifecycle_id == current_lifecycle_id()` check it replaces: the lifecycle
# whose proposal period is open right now is precisely the one whose start
# slot falls in the current epoch. Keying on the epoch rather than on the id
# is simply the property that actually makes the hash valid, and it stays
# correct if a deployment ever runs periods shorter than one epoch, where
# several lifecycle ids share a single epoch's ledger and the id that is
# "current" is not the only one entitled to that hash.
hash_from_daemon() {
  lifecycle_id=$1
